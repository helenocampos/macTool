...
public class MACTool {
    ...
    public final String CONFLICT_HEADER[] = {"Hash", "Devs intersection", "Conflicted files",
        "Conflicted files caused by same dev", "Null files", "Conflicted files caused by same dev / Conflicted files"};

    public final String GENERAL_HEADER[] = {"Hash", "Merge timestamp", "Branching time", "Merge isolation time",
    ...
    private HashSet<String> mergesList;
    private String outputPath;
    private String repositoryPath;

    public MACTool(HashSet<String> mergesList, String outputPath, String repositoryPath) {
        this.mergesList = mergesList;
    ...
    public void runAnalysis() {
        this.repos = new RepositoryDao(new File(this.repositoryPath)).getRepository();
        ...
        MergeCommitsDao mergeCommitsDao = new MergeCommitsDao(this.repos.getProject());
...
//        List<String> merges = repos.getListOfMerges();
        int max = this.mergesList.size(), cont = 0;

        for (String hashMerge : this.mergesList) {
            ...
            merge.setFilesOnBranchOne(new EditedFilesDao().getFiles(merge.getHashBase(), merge.getParents()[0], this.repos.getProject(), "All Files"));
            merge.setFilesOnBranchTwo(new EditedFilesDao().getFiles(merge.getHashBase(), merge.getParents()[1], this.repos.getProject(), "All Files"));
            ...
            //=================// Tempo de isolamento //=================//
            /**
             * Tempo de isolamento Ancestor: tempo do commit Base+1 até cada
             * Parent. Tempo de isolamento Parent: tempo do commit Base até cada
             * Parent; Tempo de isolamento Merge: tempo do commit Base até o
             * Merge.
             *
             */
            ...
            MergeCommits mergeCommits = new MergeCommits(hashMerge, this.repos.getProject());
            ...
            //================// Conflitos //==============//
            int arquivos = RevisionAnalyzer.hasConflictNum(this.repos.getProject().toString(), merge.getParents()[0], merge.getParents()[1]);
            ...
            if (arquivos > 0) //Se o número de arquivos em conflito for positivo 
            {
                //=======// Chunks //=======//
                conflito = true;
                for (String line : RunGit.getListOfResult("git diff", this.repos.getProject())) {
                ...
                List<String> arquivosList = RunGit.getListOfResult("git diff --name-only --diff-filter=U", this.repos.getProject());
        ...
        try {
            Export.toCSV(this.repos, "conflict", CONFLICT_HEADER, linesConflito, this.outputPath);
            Export.toCSV(this.repos, "general", GENERAL_HEADER, linesGeral, this.outputPath);
    ...
    public void checkWhoCausedConflict(String hashBase, String hashParent, HashMap<String, ConflictedFile> ac, int ramoID) {
    ...
    public boolean checkDeletedFile(String file) {
        try {
    ...
    public String getCommitAuthor(String hash) {
    ...
    public String getAncestorIsolationTime(String hashBase, String hashParent) {
